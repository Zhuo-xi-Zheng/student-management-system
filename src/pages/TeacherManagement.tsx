...
export default function TeacherManagement() {
  ...
  // 提交添加教师
  const submitAddTeacher = async (e: React.FormEvent) => {
    e.preventDefault();
    
    // 表单验证
    if (!formData.name || !formData.age || !formData.gender || !formData.department || !formData.teacher_id) {
  ...
  // 提交编辑教师
  const submitEditTeacher = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      // 尝试调用实际API
      const response = await fetch(`/api/teachers/${(currentTeacher as Teacher).id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      setTeachers(prev => prev.map(teacher => 
        teacher.id === (currentTeacher as Teacher).id ? updatedTeacher : teacher
      ));
      toast.success('教师信息更新成功（离线模式）');
    } finally {
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {/* 教师列表 */}{loading ? (
              ...
              <div className={`p-8 rounded-xl shadow-md ${isDark ? 'bg-gray-800' : 'bg-white'} flex flex-col items-center justify-center`}>
                ...
                <p className="text-gray-500 dark:text-gray-400">没有找到匹配的教师</p>
              </div>
            ) : (
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className={`relative w-full max-w-md mx-4 p-6 rounded-xl shadow-xl ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
            ...
            <h3 className="text-lg font-semibold mb-4">添加教师</h3>
            <form onSubmit={submitAddTeacher} className="space-y-4">
              <div>
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className={`relative w-full max-w-md mx-4 p-6 rounded-xl shadow-xl ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
            ...
            <h3 className="text-lg font-semibold mb-4">编辑教师</h3>
            <form onSubmit={submitEditTeacher} className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">姓名</label>
                <input
                  type="text"
